...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (dateItems) {
        dateItems.forEach(dateItem => {
            dateItem.textContent = new Date().toISOString().slice(0, 10).split('-').reverse().join('.')
        })
    }
    ...
    async function genSubgroupList(department, form, group) {
        ...
        let resu = ''
        response = response.forEach((subgroup, i) => {
            resu += `<label class="checkbox">
                <input type="checkbox"
                    name="other"
                    value="${i}"
            ...
            click: async function (swiper, event) {
                ...
                if (target.classList.contains('main__btn')) {
                    if (target.id == "btn2") {
                        ...
                        document.getElementById('subgoups-picker').innerHTML = await genSubgroupList(department, form, group);
                    }
                    if (target.id == "btn3") {
                        getResult()
                    }
    ...
    function getAlert() {
        ...
        alert.textContent = "Пожалуйста, заполните данные о вашем образовании перед переходом на следующий шаг!";
    ...
    function getResult() {
        ...
        document.getElementById('main__link').textContent = link.toLowerCase();
        // if (subgroup != '') {
